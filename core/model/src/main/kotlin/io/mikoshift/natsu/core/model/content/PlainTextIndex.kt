...
class PlainTextIndex private constructor(
    val plainText: String,
    private val anchors: List<BlockAnchor>,
) {
    ...
    fun readerBlockIndex(blocks: List<ReaderBlock>, position: ReadingPosition): Int {
        return blocks.indexOfFirst {
    ...
    companion object {
        fun fromDocumentPackage(documentPackage: DocumentPackage): PlainTextIndex {
            ...
            val adjustedAnchors = if (trimmedLengthDelta > 0) {
                anchors.mapNotNull { anchor ->
                    if (anchor.globalStartOffset >= plainText.length) {
                        null
                    } else {
                        val maxLength = (plainText.length - anchor.globalStartOffset)
