...
@Singleton
class PackageParser @Inject constructor(
    private val networkFactory: NetworkFactory,
    private val packageFileStore: PackageFileStore,
) {
    suspend fun parse(documentId: String): Result<DocumentPackage> =
    ...
    internal fun parseZip(zipFile: File): DocumentPackage {
        ZipFile(zipFile).use { zip ->
            ...
            val manifestDto = networkFactory.json.decodeFromString(
            ...
            val sections = manifestDto.sections.associate { section ->
                val entry = zip.getEntry(section.path)
                    ?: throw DocumentError.Unknown("Missing section file ${section.path}")
                val sectionBytes = zip.getInputStream(entry).use { it.readBytes() }
                val blocks = networkFactory.json.decodeFromString(
